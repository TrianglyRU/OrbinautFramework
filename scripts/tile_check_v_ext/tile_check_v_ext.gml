/// @self
/// @description												Checks for tile collision vertically and returns an array containing the distance to the nearest surface and its angle.
...
/// @returns {Array<Real>}
function tile_check_v_ext(_x, _y, _dir, _secondary_layer = COLLISION_LAYER.PATH_A, _quadrant = QUADRANT.DOWN)
{
	_x = floor(_x);
	_y = floor(_y);
...
function _calc_distance_v(_x, _y, _dir, _secondary_layer, _quadrant)
{
	...
	if _dir > 0
	{
	    return _tile_y + TILE_SIZE - _height - _y - 1;
	}
	else
	{
	    return _y - _tile_y - _height;
	}
}
