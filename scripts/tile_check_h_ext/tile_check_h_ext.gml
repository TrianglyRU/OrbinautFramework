/// @self
/// @description												Checks for tile collision horizontally and returns an array containing the distance to the nearest surface and its angle.
...
/// @returns {Array<Real>}
function tile_check_h_ext(_x, _y, _dir, _secondary_layer = COLLISION_LAYER.PATH_A, _quadrant = QUADRANT.DOWN)
{
	_x = floor(_x);
	_y = floor(_y);
	_y = max(0, _y);
	...
	var _angle = TILE_EMPTY_ANGLE;
	
	var _sx = _x + _dir * (_distance + 1);
...
function _calc_distance_h(_x, _y, _dir, _secondary_layer, _quadrant)
{
	...
	if _dir > 0
	{
	    return _tile_x + TILE_SIZE - _width - _x - 1;
	}
	else
	{
	    return _x - _tile_x - _width;
	}
}
