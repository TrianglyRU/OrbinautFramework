...
uniform sampler2D u_context_mask;
uniform vec2 u_context_mask_size;
...
float getContext()
{
    vec2 uv = gl_FragCoord.xy / u_context_mask_size;
    return texture2D(u_context_mask, uv).r;
}
...
void main()
{
    ...
    if (getContext() < 0.5)
    {
        swapped = getSwappedColour(base, u_texture_a, u_texel_a, u_uv_a);
    }
    else
    {
        swapped = getSwappedColour(base, u_texture_b, u_texel_b, u_uv_b);
    }
